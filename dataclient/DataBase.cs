    ...
    class ConnectDatabase
    {
        ...
        private bool ConnectionOpen()
        {
            ...
            catch (MySqlException ex)
            {
                ...
                {
                    case 0:
                        myLog.Write("無法連接到伺服器" + ex.ToString());
        ...
        //執行查詢語法，回傳搜尋目標是否存在
        public bool F_SQL_Select(String TableName, String ColumnName, String Operator, String Values)
        {
            if (ConnectionOpen() == true)//開啟連結
            {
                try
        ...
        //修改語法
        public void F_SQL_Update(String TableName, String Sel_Col_Name, String Sel_Col_Value, String Chg_Col_Name, String Chg_Col_Value)
        {
            if (ConnectionOpen() == true)//開啟連結
            {
                try
                {
                    MySqlCommand command = MysqlConnection.CreateCommand();
                    command.CommandText = "Update " + TableName + " SET " + Chg_Col_Name + "=" + Chg_Col_Value + " WHERE " + Sel_Col_Name + "=" + Sel_Col_Value;
                    command.ExecuteNonQuery();
                }
                catch (Exception ex)
                {
        ...
        //找出並回傳訂單    order_states=1為未平倉  2為已平倉
        public List<Order_Entrust> F_SQL_SelectEntrust(int userid,int order_states)
        {
            ...
            if (ConnectionOpen() == true)//開啟連結
            {
                try
                {
                    String cmdText = "SELECT * FROM  order_entrust WHERE  user_id = " + userid.ToString() + " and order_status ="+order_states.ToString();
                    MySqlCommand cmd = new MySqlCommand(cmdText, MysqlConnection);
                    MySqlDataReader reader = cmd.ExecuteReader(); //execure the reader
        ...
        //找到並回傳使用者未成交委託
        public List<Pre_Order> F_SQL_SelectPre_Order(int userid)
        {
            ...
            if (ConnectionOpen() == true)//開啟連結
            {
                try
                {
                    String cmdText = "SELECT * FROM  pre_order WHERE  user_id = " + userid.ToString() + " and state = 0 or state = 3";
                    MySqlCommand cmd = new MySqlCommand(cmdText, MysqlConnection);
                    MySqlDataReader reader = cmd.ExecuteReader(); //execure the reader
                    while (reader.Read())
                    {
